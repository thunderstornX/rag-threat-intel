...
\subsection{MRR}
For the CVE-recall subset (where ground-truth
\texttt{expected\_relevant\_source\_ids} is well-defined), we report
mean reciprocal rank at depths 5 and 10 per
chunking strategy, following the TREC-8 QA convention introduced by
Voorhees~\cite{voorhees1999trec8qa}. The harness retrieves once per
strategy and computes per-query reciprocal rank server-side; output
is a CSV (one row per (strategy, query)) plus a JSON summary.
